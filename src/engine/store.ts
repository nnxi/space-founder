...
export class PlanetStore {
  ...
  // 역할(Role)에 따라 숫자 ID 기반 조회를 분리하여 ID 충돌 방지
  private readonly userNumericToId = new Map<number, string>();
  private readonly nasaNumericToId = new Map<number, string>();
  ...
  clear(): void {
    ...
    this.userNumericToId.clear();
    this.nasaNumericToId.clear();
    this.grid.clear();
  }
  ...
  setPlanet(planet: Planet, numericId: number): void {
    ...
    if (p.role === "default") {
      this.nasaNumericToId.set(numericId, planet.id);
    } else {
      this.userNumericToId.set(numericId, planet.id);
    }
  ...
  removePlanet(planetId: string): void {
    ...
    if (numericId !== undefined) {
      if (planet && (planet as any).role === "default") {
        this.nasaNumericToId.delete(numericId);
      } else {
        this.userNumericToId.delete(numericId);
      }
    }
  ...
  getPlanetIdByNumeric(numericId: number, role: string = "user"): string | undefined {
    return role === "default" 
      ? this.nasaNumericToId.get(numericId) 
      : this.userNumericToId.get(numericId);
  }
